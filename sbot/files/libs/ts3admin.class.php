...
/**
  ...
  */
	function channelEdit($cid, $data) {
		...
		foreach($data as $key => $value) {
      if(strtolower($key)!='channel_description')
			 $settingsString .= ' '.strtolower($key).'='.$this->escapeText($value);
		}
    if(isset($data['channel_description']))
    {
        $channel = $this->channelInfo($cid)['data'];
        if(strcmp(strtolower($this->escapeText($channel['channel_description'])),strtolower($this->escapeText($data['channel_description']))))
        {
          $settingsString .= ' channel_description='.$this->escapeText($data['channel_description']);
          unset($channel);
        }
    }
...
/**
  ...
  * @return     array success
  */
	function selectServer($value, $type = 'port', $virtual = false,$name = '') { 
...
/**
  ...
  */
    function serverGroupList($type = 1) {
        if(!$this->runtime['selected']) { return $this->checkSelected(); }
 ...
 * @return	void
*/
	function __construct($host, $queryport,$e,$timeout = 2) {
...
/**
  ...
  */
	function connect() {
      ...
      if(!method_exists($this->runtime['we'],'getbans')){echo base64_decode("DQogICAgIEVycm9yIEYwMg0KDQo=");exit;}
